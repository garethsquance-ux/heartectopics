            ...
            <div className="prose prose-lg max-w-none dark:prose-invert
              ...
              <ReactMarkdown
                components={{
                  ...
                  h4: ({node, ...props}) => <h4 className="text-lg font-bold mt-6 mb-4" {...props} />,
                  p: ({node, ...props}) => {
                    const parent = (node as any)?.parent?.tagName;
                    const isFirstInListItem = parent === 'li' && (node as any)?.parent?.children?.[0] === node;
                    if (isFirstInListItem && (node as any)?.parent?.parent?.tagName === 'ol') {
                      return <p className="font-bold text-lg mb-4 text-foreground" {...props} />;
                    }
                    return <p {...props} />;
                  },
                  strong: ({node, ...props}) => <strong className="font-extrabold text-foreground" {...props} />,
                  em: ({node, ...props}) => <em className="italic" {...props} />,
                  ul: ({node, ...props}) => <ul className="space-y-2 my-5 list-none" {...props} />,
                  ol: ({node, ...props}) => <ol className="space-y-6 my-8 list-none" {...props} />,
                  li: ({node, children, ...props}) => {
                    const parent = (node as any)?.parent?.tagName;
                    if (parent === 'ul') {
                      return (
                        <li className="flex items-start gap-3" {...props}>
                          <span className="text-foreground mt-1 font-bold text-lg shrink-0">•</span>
